...
privsTopics.get = async function (tid, uid) {
  ...
  const [userPrivileges, isAdministrator, isModerator, isInstructor, disabled] = await Promise.all([helpers.isAllowedTo(privs, uid, topicData.cid), user.isAdministrator(uid), user.isModerator(uid, topicData.cid), user.isInstructor(uid), categories.getCategoryField(topicData.cid, 'disabled')]);
...
privsTopics.filterUids = async function (privilege, tid, uids) {
  ...
  const uidsResult = uids.filter((uid, index) => !disabled && (allowedTo[index] && (topicData.scheduled || !topicData.deleted) || isAdmins[index]));
  // Assert function return types in the body
