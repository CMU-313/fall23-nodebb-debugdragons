...
const privsCategories = {
  _coreSize: 0,
  getUserPrivilegeList: () => __awaiter(void 0, void 0, void 0, function* () {
    return yield plugins_1.default.hooks.fire('filter:privileges.list', Array.from(_privilegeMap.keys()));
  }),
  getGroupPrivilegeList: () => __awaiter(void 0, void 0, void 0, function* () {
    return yield plugins_1.default.hooks.fire('filter:privileges.groups.list', Array.from(_privilegeMap.keys()).map(privilege => `groups:${privilege}`));
  }),
  ...
  get: function (cid, uid) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const [userPrivileges, isAdministrator, isModerator] = yield Promise.all([helpers_1.default.isAllowedTo(privs, uid, cid), user_1.default.isAdministrator(uid), user_1.default.isModerator(uid, cid)]);
      const combined = userPrivileges.map(allowed => allowed || isAdministrator);
      const privData = _.zipObject(privs, combined);
      const isAdminOrMod = isAdministrator || isModerator;
      return yield plugins_1.default.hooks.fire('filter:privileges.categories.get', Object.assign(Object.assign({}, privData), {
        cid: cid,
        uid: uid,
  ...
  isAdminOrMod: function (cid, uid) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const [isAdmin, isMod] = yield Promise.all([user_1.default.isAdministrator(uid), user_1.default.isModerator(uid, cid)]);
  ...
  can: function (privilege, cid, uid) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      // The next line calls a function in a module that has not been updated to TS yet
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
      categories_1.default.getCategoryField(cid, 'disabled'), user_1.default.isAdministrator(uid),
      // The next line calls a function in a module that has not been updated to TS yet
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
  ...
  filterCids: function (privilege, cids, uid) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      // The next line calls a function in a module that has not been updated to TS yet
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
      categories_1.default.getCategoriesFields(cids, ['disabled']), helpers_1.default.isAllowedTo(privilege, uid, cids), user_1.default.isAdministrator(uid)]);
  ...
  filterUids: function (privilege, cid, uids) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      uids = _.uniq(uids);
      const [allowedTo, isAdmins] = yield Promise.all([helpers_1.default.isUsersAllowedTo(privilege, uids, cid), user_1.default.isAdministrator(uids)]);
      return uids.filter((uid, index) => allowedTo[index] || isAdmins[index]);
    });
  },
  give(privileges, cid, members, callback) {
    helpers_1.default.giveOrRescind(groups_1.default.join, privileges, cid, members).then(() => plugins_1.default.hooks.fire('action:privileges.categories.give', {
      privileges,
  ...
  rescind(privileges, cid, members, callback) {
    helpers_1.default.giveOrRescind(groups_1.default.leave, privileges, cid, members).then(() => plugins_1.default.hooks.fire('action:privileges.categories.rescind', {
      privileges,
      cids: Array.isArray(cid) ? cid : [cid],
      members: Array.isArray(members) ? members : [members]
    })).then(() => callback(null)).catch(error => callback(error));
  ...
  canMoveAllTopics: function (currentCid, targetCid, uid) {
    return __awaiter(this, void 0, void 0, function* () {
      const [isAdmin, isModerators] = yield Promise.all([user_1.default.isAdministrator(uid), user_1.default.isModerator(uid, [currentCid, targetCid])]);
